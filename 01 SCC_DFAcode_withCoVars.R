...
sub_data<-ewidata[which(ewidata$year>1980&ewidata$year<=max_year),] 
...
#Create separate data frames for each env variable (I know this can be done more efficiently)
##sst
...
i = 1 # ran with 1, 2, 3 for the 3 best models #7 = ild
if(model_df$var_index[i]=="equal") varIndx = rep(1,nspp)
if(model_df$var_index[i]=="unequal") varIndx = seq(1,nspp)
if(model_df$var_index[i]=="survey") varIndx = c(rep(1,14), rep(2, 13), rep(3, 8), rep(4,3))
...
saveRDS(fit.mod, file = paste0("rds_files/results_covariates/",model_df$cov[i],"_",model_df$num_trends[i],
  "_",model_df$var_index[i],"_",str,theta_str,phi_str,sigma_str,"central.rds"))
...
for(i in 1:nrow(model_df)) {
  ...
  #saveRDS(fit.mod, file = paste0("results_biology/biology_",model_df$num_trends[i],
  #  "_",var_index,"_",str,theta_str,phi_str,sigma_str,".rds"))
  saveRDS(model_df, file="/results_covariates/covariate_model_summary_null.rds")
